...
int Init()
{
	...
	c8	title[100] = "AhmadENGINE - Shaders ";
	...
	// Add a texture from photo surface
	skin->AddTexture(skinID2, photo);
	...
	// Buat sebuah font dengan menggunakan id nID
	u32 nID;

	// Register sebuah font arial bold berukuran 20, gunakan nID untuk mengaktifkan font
	g_Device->RegisterFont("Times New Roman", true, false, false, 20, &nID);
	...
	ITexture *tex = g_Device->CreateTexture(box);
	...
	g_Device->SetBackfaceCulling(ECM_CW);
		
	u32 sid1;
		
	CreateCube(IVector3f(0.0f, 0.0f, 0.0f), 8.0f, 8.0f, 8.0f, verts, indis, true);
	render->CreateStaticBuffer(EVT_VERTEXNORMAL, verts, 24, indis, 42, skinID2, &sid1);
	
	u32 vid, pid;
	g_Device->CreateVertexShader("../media/multitexture.vsh", false, &vid);
	g_Device->CreatePixelShader("../media/multitexture.psh", false, &pid);
	//g_Device->EnableShaders(false);
		...
		{	
			//if (g_bActive)	// draw only when it is focused, otherwise it will slow down our Windows
			{
				...
				{				
					...
					// Translate our cube by setting the world matrix
					mat1.Translate(5, 0, 0);
					g_Device->SetWorldMatrix(&mat1);

					// Render our cube using Dynamic Vertex Cache
					g_Device->DrawCube(IVector3f(0.0f, 0.0f, 0.0f), 6,6,6, false, skinID1);
					...
					// draw the text
					g_Device->DrawText(5, 5, "Perspective Projection - (Static && Dynamic Vertex Cache)" );
